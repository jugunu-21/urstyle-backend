...
exports.userService = {
    ...
    addLikeToUser: async ({ userId, createdLikeId }) => await models_1.User.findByIdAndUpdate(userId, { $push: { likes: createdLikeId } }, { new: true }),
    removeLikeFromUser: async (collectionId, likeId, session) => {
        await models_1.User.findOneAndUpdate({ _id: collectionId }, { $pull: { likes: { $in: [likeId] } } }, { session });
    },
